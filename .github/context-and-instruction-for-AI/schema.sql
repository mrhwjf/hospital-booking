...
('QUAN_LY_NGUOI_DUNG', 'Quản lý người dùng', 'quan_tri'),
('QUAN_LY_BAC_SI', 'Quản lý bác sĩ', 'quan_tri'),
('QUAN_LY_CHUYEN_KHOA', 'Quản lý chuyên khoa', 'quan_tri'),
('QUAN_LY_LICH_HEN', 'Quản lý lịch hẹn', 'le_tan'),
('XEM_BAO_CAO', 'Xem báo cáo', 'quan_tri'),
('CAU_HINH_HE_THONG', 'Cấu hình hệ thống', 'quan_tri'),
('XEM_NHAT_KY', 'Xem nhật ký hoạt động', 'quan_tri'),
('DAT_LICH_KHAM', 'Đặt lịch khám', 'nguoi_dung'),
('KHAM_BENH', 'Khám bệnh', 'bac_si'),
('KE_DON_THUOC', 'Kê đơn thuốc', 'bac_si');
...
BEGIN
    DECLARE next_id INT;
    SELECT COALESCE(MAX(id), 0) + 1 INTO next_id FROM benh_nhan;
...
FOR EACH ROW
BEGIN
    IF NEW.khung_gio_id IS NOT NULL THEN
...
FOR EACH ROW
BEGIN
    -- Khi check-in lần đầu: tự tạo phiếu khám ở trạng thái tiếp nhận (do nhân viên/lễ tân tạo)
    IF OLD.gio_den_thuc_te IS NULL AND NEW.gio_den_thuc_te IS NOT NULL THEN
        INSERT INTO phieu_kham (
            ...
            NEW.benh_nhan_id,
            NEW.bac_si_id,
            NEW.nguoi_tiep_nhan_id,
            NOW(),
            'tiep_nhan'
        );
    END IF;

    IF OLD.trang_thai != 'da_huy' AND NEW.trang_thai = 'da_huy' AND NEW.khung_gio_id IS NOT NULL THEN
        UPDATE khung_gio_kham 
...
-- Job tự động xóa thông báo đã đọc hơn 30 ngày
CREATE EVENT IF NOT EXISTS ev_xoa_thong_bao_cu
ON SCHEDULE EVERY 1 DAY
STARTS CURRENT_DATE + INTERVAL 1 DAY + INTERVAL 2 HOUR -- Chạy vào 2 giờ sáng mỗi ngày
DO
...
SELECT 
    ...
    ck.ten_chuyen_khoa,
    CASE
        WHEN dvlh.dich_vu_id IS NOT NULL THEN 'dich_vu'
        ELSE 'goi_kham'
    END as loai_muc,
    dv.ten_dich_vu,
    gk.ten_goi_kham,
