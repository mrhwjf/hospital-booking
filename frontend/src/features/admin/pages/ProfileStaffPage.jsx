...
export default function ProfileStaffPage() {
	...
	const staffAccountSelectOptions = useMemo(
		() =>
			staffAccountList.map((account) => ({
				value: account.id,
				label: `${account.email}${account.co_ho_so_nhan_vien ? ' • đã có hồ sơ' : ''}`,
				disabled: account.co_ho_so_nhan_vien,
			})),
		[staffAccountList]
	);
	...
	const fetchStaffAccounts = useCallback(async () => {
		...
		try {
			const items = await layDanhSachTaiKhoanNhanVien();
			setStaffAccountList(items);
		} catch {
			message.error('Không thể tải danh sách tài khoản nhân viên.');
		} finally {
						...
						<>
							<Form.Item label="Chọn tài khoản nhân viên chưa có hồ sơ">
								<Select
									...
									placeholder="Chọn tài khoản NHANVIEN"
										...
										return (
											...
											>
												...
												<div className="flex items-center gap-1">
													...
													<Tag color={account.co_ho_so_nhan_vien ? 'default' : 'processing'} className="m-0 text-[10px]">
														{account.co_ho_so_nhan_vien ? 'Đã có hồ sơ' : 'Chưa có hồ sơ'}
													</Tag>
												</div>
											</div>
										);
							...
							{!selectedExistingAccountId && (
								<div className="mb-4 rounded-lg border border-dashed border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-700">
									Vui lòng chọn tài khoản NHANVIEN chưa có hồ sơ để nhập thông tin nhân viên.
								</div>
							)}
						</>
					)}
